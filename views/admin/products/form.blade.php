        ...
        <div class="col-4">
            ...
            <form class="card card-body"
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('category_id') is-invalid @enderror" name="category_id" id="category_id">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{old('category_id', $product->category->id ?? null) == $category->id ? 'selected' : ''}}>
                                - {{ $category->name }}
                            </option>
                        @endforeach
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $product->name ?? null) }}" type="text" id="name" name="name" />
                    @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
